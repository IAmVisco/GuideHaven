...
function recreateSteps(step, header, content, images, raw_header, raw_content, imgs) {
    ...
    if (index > 0)
        stepHtml += '<button type="button" class="btn-link delete-btn" value="Delete" onclick="deleteStep(' + (index + 1) + ')">'
            + '<span class="glyphicon glyphicon-remove"></span></button></div >'
    else
        stepHtml += '</div >'

    document.getElementById("step_holder").insertAdjacentHTML('beforeend', stepHtml);
